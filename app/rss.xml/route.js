...
export async function GET(request) {
  ...
  const items = elencoDate()
    ...
    .map((d) => {
      ...
      <link>${origine}/${d}</link>
      <guid>${origine}/${d}</guid>
      <description>${esc(`Vangelo: ${meta.vangelo || '—'}. Quattro voci confessionali sotto la logica del dono.`)}</description>
...
<rss version="2.0">
  <channel>
    ...
    <link>${origine}</link>
    <description>Commento quotidiano del Vangelo da quattro voci: cattolica, ortodossa, protestante, dispensazionalista.</description>
